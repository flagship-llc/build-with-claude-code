...
Build with Claude Code ワークショップに参加するための環境構築手順です。
**なるべく事前に完了しておいてください。** セットアップ会でもフォローしますが、事前に済ませておくと当日スムーズに進められます。
...
- Mac
- インターネット接続
- Claudeのアカウント（**Pro**プラン以上）
  - 無料トライアル期間あり（クレジットカード登録が必要）
  - [Claude料金ページ](https://claude.com/ja-jp/pricing)で詳細を確認
- GitHubアカウント（必須）
  - コードのバージョン管理やポートフォリオ公開に便利
  - [GitHub](https://github.com/) で無料登録できます
...
### GitHub Copilot の AI 機能を無効化する（推奨）

VS Code に GitHub Copilot が入っている場合、Claude Code と同時に動くと混乱します。セッション中は Copilot の AI 機能を無効にしておくことを推奨します。

1. `Cmd + Shift + P`（コマンドパレット）を開く
2. 「settings」と入力 →「**基本設定: ユーザー設定を開く**」を選択
3. 検索欄に `@id:chat.disableAIFeatures` と入力
4. チェックボックスを **オン** にする

これで Copilot のチャットやインライン補完が無効になり、Claude Code だけに集中できます。

> セッション後に元に戻したい場合は、同じ設定のチェックを外せば OK です。

参考: [VS CodeのAI機能を無効化する方法（DevelopersIO）](https://dev.classmethod.jp/articles/vscode-github-copilot-ai-features-disable/)
...
- [ ] GitHub Copilot の AI 機能を無効化した（`chat.disableAIFeatures` をオン）
